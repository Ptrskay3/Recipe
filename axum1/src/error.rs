...
pub enum ApiError {
    ...
    /// that the frontend can deal with.
    ...
    /// debugging. Just use `.context()` to refine the logged error.
    ///
...
impl ApiError {
    ...
    pub fn unprocessable_entity<K, V>(errors_in: impl IntoIterator<Item = (K, V)>) -> Self
    where
