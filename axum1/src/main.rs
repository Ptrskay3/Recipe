use axum1::{
    cli::cli_manager_task, config::get_config, queue::run_worker_until_stopped,
...
async fn main() -> anyhow::Result<()> {
    ...
    let cli_manager = tokio::spawn(cli_manager_task(configuration));
    ...
    tokio::select! {
        ...
        f = cli_manager => report_exit("CLI Manager", f),
    };
